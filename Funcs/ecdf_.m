function [Fout,x,Flo,Fup,D] = ecdf_(y,N_,isa,delta,varargin)
...
%[uncens] = internal.stats.parseArgs('uncen', [], delta);
...
obscumfreq = cumsum(freq .* ~cens.*delta);
...
N = totalcount - [0; totcumfreq(1:end-1)];  % N-Q (y_tau>s)

% No change in function except at a death, so remove other points
t = (D>0);
x = x(t);
D = D(t);       % Q_hat (y_tau=s, delta=1)
N = N(t);
