function [reward, regret, T_path, a, explod, F_diff] = RunAlg_sample(chanrange, Time, alg, req, Rayl, contreq, usew, wsize)
    explod = false;
    if Rayl
        K=3;
    ...
    for t = 1:Time
        ...
        if t < K
            ...
            switch alg
                case "T-UCB-LKM"
                    [aaa, F_hat] = T_UCB_LKM___(N, a, c_t, c, y, delta, t-1, N_c{c_ind}, N_tild);
                ...
                case "T-UCB"
                    if usew && t>wsize
                        ...
                        aaa = T_UCB_(F,N,t-1,N_c{c_ind},a,c);
                    end
                    F_est = F;
                case "T-UCB-C"
                    if usew && t>wsize
                        ...
                        aaa = T_UCB_(F_tild,N_tild,t-1,N_c{c_ind},a,c);
                    end
        ...
        if exist('F_est','var')
            F_diff{c_ind} = [F_diff{c_ind}; abs(F_est - nu_c)];
        end
